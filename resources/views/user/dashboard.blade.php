...
@section('page-subtitle', 'Ringkasan data pendaftaran kursus pernikahan Anda')
...
{{-- Chart: Status Pembayaran (donut) - hanya jika ada data --}}
@if($pendaftaranList->isNotEmpty() && ($stats['lunas'] + $stats['menunggu'] + $stats['belum_bayar']) > 0)
<div class="charts-row charts-row-1">
    <div class="chart-card">
        <div class="chart-header">
            <h3>Status Pembayaran Anda</h3>
            <p>Distribusi status pembayaran pendaftaran Anda</p>
        </div>
...
<div class="card">
    <div class="card-header">
        <div>
            ...
            <p>Daftar pendaftaran kursus pernikahan atas nama Anda</p>
        </div>
        <a href="{{ route('kursus-pernikahan') }}" class="btn-sm">
            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="td-names">
                                ...
                                <small>{{ $item->tanggal_pernikahan ? $item->tanggal_pernikahan->locale('id')->isoFormat('D MMM YYYY') : '—' }} · {{ $item->tempat_pernikahan ?? '—' }}</small>
                            </div>
                        </td>
    ...
    @else
        <div class="empty-state">
            ...
            <a href="{{ route('kursus-pernikahan') }}" class="btn-sm" style="margin-top:14px;display:inline-flex">
    ...
    new Chart(document.getElementById('chartPembayaran'), {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: { callbacks: { label: ctx => ` ${ctx.label}: ${ctx.parsed} pendaftaran` } }
            },
        }
    });
})();
